...
def read_topo():
    ...
    nb_switches = 0
    links = []
    with open("mappers_topo.txt", "r") as f:
        line = f.readline()[:-1]
...
def word_flags_assemble(msg, msg_size, pkt_type):
    if pkt_type == '0':
        l = msg
        # Paddings
        while len(l) < msg_size:
            l += 'o'
        l = l[:msg_size]
        return l + chr(msg_size) + chr(0)
    elif pkt_type == '1':
        return msg_size*chr(0) + chr(msg_size) + chr(1)
    else:
        print "Unexpected error:", sys.exc_info()[0]
        raise
...
def main():
    ...
    while(1):
        ...
        FIX_WORD_SIZE = 5
        p = SrcRoute() / \
                word_flags_assemble(msg, FIX_WORD_SIZE, pkt_type) / \
                msg
                # port_str / \
