...
action set_port() {
 ...
 modify_field_with_hash_based_offset(standard_metadata.egress_spec,
                                     PORT_INDEX_BASE,word_hashing_spec,
                                     PORT_INDEX_SIZE); 
}
...
action send_to_all() {
  ...
  clone_i2i(10, egress_port_number);
}
...
control egress {
  if(valid(word_header) and
      word_header.flags != 0x00 and
      egress_metadata.port_number < PORT_INDEX_BASE + PORT_INDEX_SIZE -1) { 
       // send to all reducers a request to their tables
