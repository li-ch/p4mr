...
register word_map_pointer { // there is only one register which points to the next cell of word_counter to send in a new packet
  width : NUM_OF_BITS_FOR_INDEX;
  instance_count : 1; // one pointer 
}
...
action start_processing() {
  ...
  register_read(ingress_metadata.word_map_representation, word_value, 0);
  register_read(ingress_metadata.word_counter_representation, word_counter, 0);   
  shift_left(ingress_metadata.word_map_representation, ingress_metadata.word_map_representation, 5360);
  shift_left(ingress_metadata.word_counter_representation, ingress_metadata.word_counter_representation, 4288);  
...
action send_switch_map() {
  ...
  register_read(ingress_metadata.word_pointer_current_value, word_map_pointer, 0); // only one instance
   ...
   // read stored values from the registers
  register_read(ingress_metadata.word_map_representation, word_value, ingress_metadata.word_pointer_current_value);
  register_read(ingress_metadata.word_counter_representation, word_counter, ingress_metadata.word_pointer_current_value);   
...
control ingress {
   ...
   if (valid(word_map_header) and word_map_header.num_of_words < MAX_NUM_OF_WORDS)
   {
      apply(resubmit_table);
   }
   else // set output port number
   {
      apply(port_number_table);
   }
